...
- Persistent one-directional PlayerBot-to-real-player sentiment, with a `-100…100` score, C++-derived tiers, bounded model deltas, lazy asymmetric decay, bounded lazy RAM/SQL storage, and private moderator controls.
...
| Bounded relationship changes | The suggested limits are appropriate and are hard bounds, not prompt-only advice. | A few output tokens | Low | Ordinary dialogue is clamped to `±2`; verified event reactions are clamped to `±3`; total score is clamped to `-100…100`. | A malicious `+999` marker becomes `+2` in ordinary chat. |
...
| Sentiment in random/events | Random chatter uses the anchor's tier without changing it; verified event reactions may produce a bounded `±3` change. | Low | Medium | `Sentiment.UseInRandom` and `Sentiment.UseInEvents` gate these paths. Bot-subject events with no unique real-player counterpart create no pair. | A bot's ambient line near Alice can sound warm; Alice's quest event may move the score. |
...
3. Reuse the existing dialogue response for one stripped, hard-clamped delta; use `±2` for conversation, `±3` for server-verified events, and no Random-trigger mutation.
...
Use generic actor_guid/target_guid SQL keys, one clamped -100..100 integer, and
...
conversation changes to +/-2 and verified event changes to +/-3; Random may use
...
when SQL is unavailable. Keep exact private GM inspect/set/pair-reset/reset-all
commands, with reset-all explicitly separate.
...
The 2 September 2026 source audit used the checked-out Windows x64 Visual Studio 2022 build configured with PlayerBots enabled, Turtle addons enabled, extractors disabled, and static modules. CMake discovered `AzerothVoicesSentiment.cpp`, generated a static loader that calls `Addmod_azeroth_voicesScripts`, and generated install rules for the module config, data tree, and README. Standalone C++ boundary checks passed for score clamping, all tier edges, valid and malformed metadata stripping, `±2`/`±3` delta clamping, and tier-only prompt text. The complete configured Release solution built successfully; after the final configuration-bound fix, the Release `mangosd` target rebuilt successfully at `bin/Release/mangosd.exe`.
...
`AzerothVoices.Sentiment.Enable = 1` tracks only one direction: an AI-controlled PlayerBot's regard toward a real player. A pair is created or changed only after a valid hidden model delta accompanies a successfully delivered first line, or through an exact moderator `set` command. NPC actors and targets are excluded, PlayerBot targets are excluded, and generated PlayerBot/NPC follow-ups do not invent a real-player counterpart. Random chatter can read the relationship toward its real-player anchor but never changes the score. Event chatter can read and change it only when the event subject is a unique real player.
...
Ordinary conversation asks the same provider response for a final `[[AV_SENTIMENT:N]]` metadata line, with `N` hard-clamped to the configured `-2…+2` range. Event reactions use the separate `-3…+3` maximum. The marker is removed before reply splitting, generated-message logging, delivery, surrounding chat, and history. Missing, invalid, or truncated metadata does not fail the dialogue and does not change the relationship. No classifier request, embeddings request, Python bridge, or second API call is used.

Defaults are:

```ini
AzerothVoices.Sentiment.Enable = 1
AzerothVoices.Sentiment.UseInRandom = 1
AzerothVoices.Sentiment.UseInEvents = 1
AzerothVoices.Sentiment.ConversationMaximumDelta = 2
AzerothVoices.Sentiment.MajorEventMaximumDelta = 3
