...
namespace AzerothVoices
{
                                          ...
                                          uint32_t deltaLimit)
    {
        ...
        if (deltaLimit)
        {
            block << " End with [[AV_SENTIMENT:N]] on its own line, where N is one integer from -"
                  << deltaLimit << " to +" << deltaLimit
                  << " for the regard change caused by this player's words or verified event; use 0 if none. "
    ...
    bool ExtractSentimentDelta(std::string& response, uint32_t deltaLimit, int32_t& delta)
    {
        ...
        while ((search = response.find(prefix, search)) != std::string::npos)
        {
            ...
            {
                int32_t const limit = static_cast<int32_t>(std::min<uint32_t>(deltaLimit, 3));
